...
#' of `x` is assumed to be provided in natural order already (see *Details*).
#' @export
#' @rdname as_rvine_structure
as_rvine_structure.list <- function(x, ..., is_natural_order = TRUE) {

    assert_that(is.list(x),
                all(c("order", "struct_array") %in% names(x)),
                is.flag(is_natural_order))
    
    rvine_structure(x[["order"]], 
                    x[["struct_array"]], 
                    is_natural_order)
}
...
#' @export
#' @rdname as_rvine_structure
as_rvine_matrix.list <- function(x, ..., is_natural_order = TRUE) {

    assert_that(is.list(x),
                all(c("order", "struct_array") %in% names(x)),
                is.flag(is_natural_order))
    
    rvine_struct <- rvine_structure(x[["order"]], 
                                    x[["struct_array"]], 
                                    is_natural_order)
...
as_rvine_structure.rvine_matrix <- function(x, ..., validate = FALSE) {
    ...
    # create and return x
    structure(list(order = diag(x[,d:1]), 
