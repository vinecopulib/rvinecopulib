...
#' An example array is ``` 4 4 4 4 3 3 3 2 2 1 ``` which encodes the following
#' pair-copulas:
#'
#' \tabular{lll}{ tree \tab  edge \tab pair-copulas   \cr 0    \tab  0    \tab
#' `(1, 4)`       \cr \tab  1    \tab `(2, 4)`       \cr \tab  2    \tab `(3,
#' 4)`       \cr 1    \tab  0    \tab `(1, 3; 2)`    \cr \tab  1    \tab `(2, 3;
#' 4)`    \cr 2    \tab  0    \tab `(1, 2; 3, 4)` }
#'
...
#' ``` 4 4 4 4 3 3 3 0 2 2 0 0 1 0 0 0 ``` Similarly, an R-vine matrix can be
#' converted to an R-vine structure using [as_rvine_structure()].
#'
#' Denoting by `M[i, j]` the array entry in row `i` and column `j` (the
#' pair-copula index for edge `e` in tree `t` of a `d` dimensional vine is `(M[d
#' + 1 - e, e], M[t, e]; M[t - 1, e], ..., M[1, e])`. Less formally, 1. Start
#' with the counter-diagonal element of column `e` (first conditioned variable).
#' 2. Jump up to the element in row `t` (second conditioned variable). 3. Gather
#' all entries further up in column `e` (conditioning set).
#'
...
#' elements off the structure above are stored as ``` 4 4 4 3 3 2 ``` for the
#' structure above. The reason is that it allows for parsimonious
...
#' (see [as_rvine_structure()] and `as_rvine_matrix(`) are called: 1. It can
#' only contain numbers between 1 and d (and additionally zeros for R-vine
#' matrices). 3. The anti-diagonal must contain the numbers 1, ..., d. 4. The
#' anti-diagonal entry of a column must not be contained in any column further
#' to the right. 5. The entries of a column must be contained in all columns to
#' the left. 6. The proximity condition must hold: For all t = 1, ..., d - 2 and
#' e = 1, ..., d - t there must exist an index j > d, such that `(M[t, e], {M[1,
#' e], ..., M[t - 1, e]})` equals either `(M[d + 1 - j, j], {M[1, j], ..., M[t -
#' 1, j]})` or `(M[t - 1, j], {M[d + 1 - j, j], M[1, j], ..., M[t - 2, j]})`.
#'
#' Condition 6 already implies conditions 2-5, but is more difficult to check by
#' hand.
#'
