...
exposes the same structure control through `copula_controls`.
[`rvine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine_methods.md)
accepts `x_cond` on the original scale and uses the fitted margins to
...
[`rvine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine_methods.md)
handles this internally from `x_cond`, including ordered and
...
random-number state. Call
[`set.seed()`](https://rdrr.io/r/base/Random.html) immediately before
the operation when a reproducible draw is required. The original model
