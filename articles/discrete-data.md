...
interface computes these values from fitted margins. The copula-only
...
interfaces accept them explicitly.
...
No `var_types` entry is needed because
[`ordered()`](https://rdrr.io/r/base/factor.html) carries the
declaration in the column class.

## Zero-inflated variables

[`zero_inflated()`](https://vinecopulib.github.io/rvinecopulib/reference/zero_inflated.md)
marks a continuous variable with an atom at zero. It behaves like a
numeric vector in a data frame and survives ordinary subsetting.
...
return the copula density contribution defined as the joint density or
mass divided by the marginal densities or masses. This definition is
consistent across continuous, discrete, and mixed models.
...
multiplies that contribution by the fitted marginal densities or masses
and therefore returns the full joint density or probability mass on the
...
always returns one uniform-scale value per variable. It does not invent
a marginal distribution for a discrete copula variable. Use
[`rvine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine_methods.md)
when simulated values should be returned on the original scale with
their integer, ordered, or zero-inflated margins.
...
randomizes uniformly within that interval so the transformed variable is
uniform under a correct model. Set `randomize_discrete = FALSE` to
return the deterministic upper endpoint.
...
fitters should either handle missing values or fail with a useful
message.
...
- [Marginal
  ...
  documents how
  [`vine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine.md)
  fits and evaluates integer, ordered, and zero-inflated margins.
- [Conditional simulation and Rosenblatt
  ...
  develops the multivariate transform and conditional-sampling workflow.
