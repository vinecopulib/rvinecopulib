...
|----|----|----|
| Original observations | [`vine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine.md) | Margins and a vine copula |
| Uniform pseudo-observations | [`vinecop()`](https://vinecopulib.github.io/rvinecopulib/reference/vinecop.md) | A vine copula |
| Two uniform variables | [`bicop()`](https://vinecopulib.github.io/rvinecopulib/reference/bicop.md) | A bivariate copula |
...
when the observations are on their original scale. It estimates a
marginal distribution for every variable, transforms the data to the
copula scale, and fits the dependence model.
...
and `predict(..., what = "cdf")` approximate a multivariate CDF by Monte
Carlo. Increase `n_mc` when accurate tail probabilities are important.

## A dependence model on the copula scale

Use
[`vinecop()`](https://vinecopulib.github.io/rvinecopulib/reference/vinecop.md)
when the margins are not part of the model. Its input consists of
approximately uniform pseudo-observations. For continuous data,
[`pseudo_obs()`](https://vinecopulib.github.io/rvinecopulib/reference/pseudo_obs.md)
applies scaled ranks column by column.
...
discrete-data article explains the accepted layouts.
[`vine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine.md)
constructs these quantities automatically from its fitted margins. See
...
for two-dimensional copula data. It estimates each compatible candidate
and selects a family using AIC by default.
...
[`fitted()`](https://rdrr.io/r/stats/fitted.values.html) need the
training data.
...
[`vinecop()`](https://vinecopulib.github.io/rvinecopulib/reference/vinecop.md)
additionally controls structure selection, truncation, and thresholding.
These options are developed in the vine-model article rather than hidden
in a single large example.

## Reproducibility and persistence

Ordinary model fitting is deterministic. Simulation uses R’s
random-number state, so
[`set.seed()`](https://rdrr.io/r/base/Random.html) makes it
reproducible. Quasi-random simulation is available through
`qrng = TRUE`.

All model objects can be stored with
[`saveRDS()`](https://rdrr.io/r/base/readRDS.html) and restored with
[`readRDS()`](https://rdrr.io/r/base/readRDS.html). Custom margin
classes remain usable after restoration as long as their S3 methods are
