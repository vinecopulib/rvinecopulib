...
separates marginal modeling from copula modeling. Each fitted margin
provides a density or probability mass function, a distribution
...
KDE controls belong to the family specification. This keeps
[`vine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine.md)
...
The `weights` argument to
[`vine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine.md)
is used for both the margins and copula. `kde1d` supports weights
directly. Every margin-family fitter receives `x`, `weights`, and
`type`; it must use the weights or reject them explicitly:
...
When no weights are supplied, `weights` is
[`numeric()`](https://rdrr.io/r/base/numeric.html). The built-in
...
wraps a fitting function with the canonical `x`, `weights`, and `type`
...
is the simplest way to construct such an object.
...
or receive them through the `fit_args` argument to
[`margin_family()`](https://vinecopulib.github.io/rvinecopulib/reference/margin_family.md).
When several candidates compete, each fitted object’s
...
the remaining fits; if all fail,
[`vine()`](https://vinecopulib.github.io/rvinecopulib/reference/vine.md)
reports the collected errors. A sole candidate without a finite
parameter count is retained with a warning, but model AIC and BIC are
then unavailable.
...
generics dispatch on their second argument, `margin`;
[`margin_info()`](https://vinecopulib.github.io/rvinecopulib/reference/margin_protocol.md)
...
that the core code interprets. See
[`?margin_protocol`](https://vinecopulib.github.io/rvinecopulib/reference/margin_protocol.md)
for the complete return-value and left-limit contract.
...
and
[`margin_info()`](https://vinecopulib.github.io/rvinecopulib/reference/margin_protocol.md)
methods. The core selection path treats such families like any other
implementation.
...
marks a numeric vector as continuous away from zero with an atom at
zero. The marker survives data-frame storage and subsetting. The same
...
returns `type = "zi"` from
[`margin_info()`](https://vinecopulib.github.io/rvinecopulib/reference/margin_protocol.md).
At zero,
[`dmargin()`](https://vinecopulib.github.io/rvinecopulib/reference/margin_protocol.md)
must return the atom probability. rvinecopulib then computes the left
limit as `F(0) - f(0)`; away from zero the left limit equals the
ordinary CDF.

## Fixed margins and persistence

[`stats_margin()`](https://vinecopulib.github.io/rvinecopulib/reference/stats_margin.md)
adapts fixed `stats` distributions, including `norm`, `lnorm`, `gamma`,
and `weibull`, to the fitted-margin protocol. Legacy `list(distr = ...)`
specifications remain accepted by
...
Fitted margins, including callback-based
[`margin_dist()`](https://vinecopulib.github.io/rvinecopulib/reference/margin_dist.md)
