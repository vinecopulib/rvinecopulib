...
)
{
    ...
    for (long j = 1; j <= n; ++j) {
        ...
        if (jpn <= npt) {
            ...
            bmat[jp + j * bmat_dim1] =
                -temp + 1.0 / ptsaux[(j << 1) + 1];
            bmat[jpn + j * bmat_dim1] =
                temp + 1.0 / ptsaux[(j << 1) + 2];
            bmat[j * bmat_dim1 + 1] = -bmat[jp + j * bmat_dim1] -
                                      bmat[jpn + j * bmat_dim1];
            zmat[j * zmat_dim1 + 1] = sqrt_2 / std::abs(
                ptsaux[(j << 1) + 1] * ptsaux[(j << 1) + 2]);
            zmat[jp + j * zmat_dim1] =
                zmat[j * zmat_dim1 + 1] * ptsaux[(j << 1) + 2] *
                temp;
            zmat[jpn + j * zmat_dim1] =
                -zmat[j * zmat_dim1 + 1] * ptsaux[(j << 1) + 1] *
                temp;
        } else {
            bmat[j * bmat_dim1 + 1] = -1.0 / ptsaux[(j << 1) + 1];
            bmat[jp + j * bmat_dim1] = 1.0 / ptsaux[(j << 1) + 1];
            bmat[j + npt + j * bmat_dim1] =
                -0.5 * square(ptsaux[(j << 1) + 1]);
        }
    }
    ...
    if (knew != kopt) {
        ...
        update(n, npt, bmat + bmat_offset, zmat + zmat_offset, ndim,
               vlag, beta, denom, knew, w);
        if (nrem == 0) {
            goto L350;
        }
        for (long k = 1; k <= npt; ++k) {
    ...
    for (long kpt = 1; kpt <= npt; ++kpt) {
        ...
        for (long i = 1; i <= n; ++i) {
            w[i] = std::min(
                std::max(xl[i], xbase[i] + xpt[kpt + i * xpt_dim1]),
                xu[i]);
        ...
        for (long k = 1; k <= npt; ++k) {
            sum = 0.0;
            for (long j = 1; j <= nptm; ++j) {
                sum += zmat[k + j * zmat_dim1] *
                       zmat[kpt + j * zmat_dim1];
            }
            temp = diff * sum;
            if (ptsid[k] == 0.0) {
                pq[k] += temp;
