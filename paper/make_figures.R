...
fit_zilnorm <- function(x, weights = numeric(), type = "zi") {
  ...
  margin_dist(
    d = function(q) ifelse(q > 0, (1 - p0) * dlnorm(q, ml, sl), q == 0) *
      ifelse(q > 0, 1, p0),
    p = function(q) ifelse(q > 0, p0 + (1 - p0) * plnorm(q, ml, sl), (q >= 0) * p0),
    q = function(p) ifelse(p <= p0, 0, qlnorm(pmax((p - p0) / (1 - p0), 0), ml, sl)),
...
par(mfrow = c(1, 3), mar = c(3.6, 3.6, 2.2, 0.8), mgp = c(2.2, 0.7, 0), bty = "n")

pos <- dat$cost[dat$cost > 0]
p0 <- 1 - mean(dat$cost > 0)
hist(log(pos), breaks = 40, freq = FALSE, col = "grey88", border = "white",
     main = "cost (zero-inflated)", xlab = "log claim amount, positive part",
     ylab = "density")
xs <- seq(min(log(pos)), max(log(pos)), length.out = 300)
lines(xs, dmargin(exp(xs), fit$margins[[1]]) * exp(xs) / (1 - p0), lwd = 2)
legend("topright", bty = "n", legend = sprintf("atom at 0: %.3f", p0), cex = 0.9)

hist(dat$veh_value, breaks = 60, freq = FALSE, col = "grey88", border = "white",
     xlim = c(0, 6), main = "veh_value (continuous)", xlab = "vehicle value",
     ylab = "density")
...
bp <- barplot(prop.table(table(as.integer(dat$agecat))), col = "grey88",
              border = "white", main = "agecat (discrete)",
              xlab = "driver age category", ylab = "probability")
...
par(mfrow = c(1, 2), mar = c(3.8, 3.8, 2.2, 0.8), mgp = c(2.4, 0.7, 0), bty = "n")
p <- c(unconditional = mean(su[, "cost"] > 0), conditional = mean(sc[, "cost"] > 0))
b <- barplot(p, ylim = c(0, 0.13), col = c("grey80", "grey35"), border = "white",
             ylab = "P(claim)", main = "probability of a claim")
text(b, p + 0.008, sprintf("%.3f", p), cex = 0.95)
du <- density(log(su[su[, "cost"] > 0, "cost"]))
dc <- density(log(sc[sc[, "cost"] > 0, "cost"]))
plot(du, lwd = 2, col = "grey55", main = "claim amount given a claim",
     xlab = "log claim amount", ylab = "density", ylim = range(0, du$y, dc$y))
lines(dc, lwd = 2)
legend("topright", bty = "n", lwd = 2, col = c("grey55", "black"),
       legend = c("unconditional", "given profile"), cex = 0.9)
dev.off()
...
full <- vinecop(u_ret, family_set = "parametric", tree_crit = "tau",
                trunc_lvl = Inf, selcrit = "mbicv", cores = 4)
...
par(mfrow = c(1, 2), mar = c(3.8, 4.0, 2.0, 0.8), mgp = c(2.5, 0.7, 0), bty = "n")
plot(ks, ll, type = "l", lwd = 2, xlab = "truncation level",
     ylab = "log-likelihood", main = "fit")
points(10, ll[10], pch = 19)
text(10, ll[10], " selected", adj = c(0, 1.6), cex = 0.9)
plot(ks, mb, type = "l", lwd = 2, xlab = "truncation level", ylab = "mBICV",
     main = "criterion")
