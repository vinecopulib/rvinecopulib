  ...
  for (rot in rotations) {
    ...
    out <- rbind(
      ...
      data.frame(
        ...
        h1 = max(abs(
          hbicop(grid, 1, rv) - BiCopHfunc1(grid[, 1], grid[, 2], vc)
        )),
        h2 = max(abs(
          hbicop(grid, 2, rv) - BiCopHfunc2(grid[, 1], grid[, 2], vc)
        )),
        hi1 = max(abs(
          hbicop(grid, 1, rv, inverse = TRUE) -
            BiCopHinv1(grid[, 1], grid[, 2], vc)
        )),
        hi2 = max(abs(
          hbicop(grid, 2, rv, inverse = TRUE) -
            BiCopHinv2(grid[, 1], grid[, 2], vc)
        )),
        tau = abs(par_to_ktau(rv) - BiCopPar2Tau(vc$family, vc$par, vc$par2)),
        beta = abs(
          blomqvist_beta(rv) - BiCopPar2Beta(vc$family, vc$par, vc$par2)
        ),
