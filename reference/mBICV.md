...
\$\$BIC = -2 loglik + \nu log(n) - 2 \sum\_{t=1}^{d - 1} (q_t
log(\psi_0^t) + (d - t - q_t) log(1 - \psi_0^t)) \$\$

where \\\mathrm{loglik}\\ is the log-likelihood and \\\nu\\ is the
(effective) number of parameters of the model, \\t\\ is the tree level
\\\psi_0\\ is the prior probability of having a non-independence copula
and \\q_t\\ is the number of non-independence copulas in tree \\t\\. The
mBICv is a consistent model selection criterion for parametric sparse
vine copula models.
