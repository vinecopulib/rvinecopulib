...
- ties_method:
  ...
  similar to `ties.method` of
  [`rank()`](https://rdrr.io/r/base/rank.html) (only `"average"`,
...
Given `n` realizations \\x_i=(x\_{i1}, \ldots,x\_{id})\\, \\i \in
\left\lbrace 1, \ldots,n \right\rbrace \\ of a random vector `X`, the
pseudo-observations are defined via \\u\_{ij}=r\_{ij}/(n+1)\\ for \\i
\in \left\lbrace 1, \ldots,n \right\rbrace\\ and \\j \in \left\lbrace 1,
\ldots,d \right\rbrace \\, where \\r\_{ij}\\ denotes the rank of
\\x\_{ij}\\ among all \\x\_{kj}\\, \\k \in \left\lbrace 1, \ldots,n
\right\rbrace \\.
...
the result by \\n/(n+1)\\. This asymptotically negligible scaling factor
is used to force the variates to fall inside the open unit hypercube,
for example, to avoid problems with density evaluation at the
boundaries.
