...
- struct_array:
  ...
  a list of vectors of positive integers. The vectors represent rows of
  the r-rvine structure and the number of elements have to be compatible
  with the `order` vector. If empty, the model is 0-truncated.
...
which encodes the same model with a square matrix filled with zeros. For
instance, the matrix corresponding to the structure above is:
...
methods (see
[`as_rvine_structure()`](https://vinecopulib.github.io/rvinecopulib/reference/as_rvine_structure.md)
and `as_rvine_matrix(`) are called:
...
5.  The proximity condition must hold: For all t = 1, ..., d - 2 and e =
    1, ..., d - t there must exist an index j \> d, such that
