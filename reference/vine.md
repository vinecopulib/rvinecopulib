...
- margins_controls:
  ...
  a list with arguments to be passed to marginal fitting. The supported
  entries are

  - `family_set` candidate families used for every variable, or a list
    with one entry per variable. The default `"kde1d"` preserves
    nonparametric margin fitting. Other character names refer to
    `univariateML` families; configured built-in and custom candidates
    are created with
  ...
  - `cores` number of cores used for marginal fitting. `NULL` inherits
    the top-level `cores` argument. Use one core for stochastic custom
    fitters when results must be invariant to the number of cores.
...
- weights:
  ...
  optional numeric vector of weights for each observation, used for both
  marginal and copula estimation. Every margin family receives these
  weights and is responsible for using or explicitly rejecting them.
...
- var_types:
  ...
  optional variable types, one for each variable after factor expansion:
  `"c"` for continuous, `"d"` for integer-valued discrete, or `"zi"` for
  continuous with an atom at zero. Types are inferred from
...
- margins:
  ...
  a list containing one marginal distribution per variable. Each margin
  can be a
  [`margin_dist()`](https://vinecopulib.github.io/rvinecopulib/reference/margin_dist.md)
  object, another fitted object implementing the [margin
  ...
  object. Legacy `list(distr = ...)` specifications are converted
  through
  [`stats_margin()`](https://vinecopulib.github.io/rvinecopulib/reference/stats_margin.md).
  If `margins` has length one, it is recycled for every component.

- pair_copulas:

  A nested list of 'bicop_dist' objects, where `pair_copulas[[t]][[e]]`
  corresponds to the pair-copula at edge `e` in tree `t`.
...
class `vine`, see
[`vinecop()`](https://vinecopulib.github.io/rvinecopulib/reference/vinecop.md).
Additionally, objects from the `vine` class contain:

- `margins_controls`, a `list` with the controls used to fit and select
...
a list with the same parameters as
[`vinecop()`](https://vinecopulib.github.io/rvinecopulib/reference/vinecop.md)
(except for `data`).
...
`univariateML` package; the default `"kde1d"`,
[`kde1d_family()`](https://vinecopulib.github.io/rvinecopulib/reference/kde1d_family.md),
and custom
[`margin_family()`](https://vinecopulib.github.io/rvinecopulib/reference/margin_family.md)
