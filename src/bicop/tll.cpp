...
namespace vinecopulib
{
    ...
    Eigen::Matrix2d TllBicop::select_bandwidth(
            const Eigen::Matrix<double, Eigen::Dynamic, 2>& x,
            std::string method
    )
    {
    ...
    Eigen::MatrixXd TllBicop::fit_local_likelihood(
            const Eigen::Matrix<double, Eigen::Dynamic, 2>& x,
    ...
    double TllBicop::calculate_infl(const size_t& n, 
                                    const double& f0, 
                                    ...
                                    const std::string& method)
    {
        ...
        if (method == "constant") {
            ...
            M(3, 3) = 0.25 * f0;
            M(3, 3) *= 3.0*Si(0, 0)*Si(0, 0) + 6.0*Si(0, 0)*b(0)*b(0) + std::pow(b(0), 4);
            M(4, 4) = 0.25 * f0;
            M(4, 4) *= 3.0*Si(1, 1)*Si(1, 1) + 6.0*Si(1, 1)*b(1)*b(1) + std::pow(b(1), 4);
                   ...
                   std::string method, double mult)
    {
        ...
        auto infl_grid = tools_interpolation::InterpolationGrid(grid_points, infl, 0);
