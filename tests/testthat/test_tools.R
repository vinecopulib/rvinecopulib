...
test_that("'pseudo_obs' works", {
  ...
  expect_equal(sort(pseudo_obs(mat)[, 1]), (1:10) / 11)
  expect_equal(sort(pseudo_obs(mat)[, 2]), (1:10) / 11)

  df <- data.frame(a = rnorm(10), b = rnorm(10))
  expect_equal(sort(pseudo_obs(df)[, 1]), (1:10) / 11)
  expect_equal(sort(pseudo_obs(df)[, 2]), (1:10) / 11)

  expect_is(pseudo_obs(x), "numeric")
  expect_is(pseudo_obs(mat), "matrix")
  expect_is(pseudo_obs(df), "data.frame")
  expect_equal(names(pseudo_obs(df)), c("a", "b"))
