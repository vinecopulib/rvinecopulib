...
several candidates compete, each fitted object needs a finite
`margin_loglik()` result; AIC and BIC additionally require a finite
`margin_npars()` result. If one candidate fails, selection continues with the
remaining fits; if all fail, `vine()` reports the collected errors.
...
Packages can integrate their own fitted classes by implementing eight S3
methods. The `dmargin()`, `pmargin()`, and `qmargin()` generics dispatch on
their second argument, `margin`; the other methods provide model metadata.
...
margin_type.my_margin <- function(margin) "c"
margin_support.my_margin <- function(margin) c(-Inf, Inf)
margin_family_name.my_margin <- function(margin) "my-family"
margin_npars.my_margin <- function(margin) margin$npars
margin_loglik.my_margin <- function(margin) margin$loglik
```
...
`fit_margin()`, `margin_family_types()`, and `margin_family_name()` methods.
This is how the built-in kde1d and univariateML adapters integrate with
`vine()`; the core selection path treats them like any other implementation.
...
returns `"zi"` from `margin_type()`. At zero, `dmargin()` must return the atom probability.
