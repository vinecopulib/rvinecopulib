...
inside `margins_controls`.

## Observation weights

The `weights` argument to `vine()` is used for both the margins and copula.
`kde1d` supports weights directly. A custom family supports them by accepting
a named `weights` argument in its fitting function:
...
When no weights are supplied, custom fitters are still called with `x` only.
With weights, a fitter that does not accept `weights` fails as an individual
candidate; selection can continue if another candidate succeeds. The built-in
univariateML adapter does not currently support observation weights.
